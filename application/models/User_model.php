...
class User_model extends CI_Model
{
    ...
    public function get_detail_guru($id, $level)
    {
        ...
        $this->db->where('tb_user.id_user', $id);
        $this->db->where('tb_user.level', $level);
        $this->db->join('tb_ustadz', "tb_user.id_user = tb_ustadz.id_user", 'left');
        return $this->db->get()->row_array();
    }

    // public function get_detail_santri($id, $level)
    // {
    //     $this->db->select('*');
    //     $this->db->from('tb_user tu');
    //     $this->db->join('tb_santri ts', "tu.id_user = ts.id_user", 'left');
    //     $this->db->join('tb_datasantri td', "ts.id_santri = td.id_santri", 'left');
    //     $this->db->join('tb_jilid tk', "td.id_jilid = tk.id_jilid", 'left');
    ...
    private function _get_datatables_query($level, $levels)
    {
        ...
        $table_join = $level == 'admin' ? 'tb_admin' : ($level == 'guru' ? 'tb_ustadz' : ($level == 'ustadz' ? 'tb_ustadz' : ($level == 'santri' ? 'tb_santri' : NULL)));
