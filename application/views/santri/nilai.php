<div class="container-fluid">
    ...
    <div class="card">
        <div class="card-body">
            <div class="form-inline">
                <select class="form-control mb-2 mr-2" id="thn_ajaran" name="thn_ajaran">
                    <?php foreach ($tahun as $th) : ?>
                        <?php foreach ($alljilid as $kl) : ?>
                            <?php if ($th->nama == $kl->tahun_ajaran) : ?>
                                <?php if ($th->nama == $kl->tahun_ajaran) : ?>
                                    <?php if ($th->id_tahun == $tahun_aktif['id_tahun']) : ?>
                                        <option value="<?php echo $th->id_tahun ?>" selected><?= $th->nama ?></option>
                                    <?php else : ?>
            ...
            <div id="data-nilai">
                <h5 class="text-right">jilid <?= $nama_jilid = ($jilid) ? $jilid['jilid'] : 'Belum Ditentukan' ; ?></h5>
...
<script>
    function lihatNilai() {
        ...
        $.ajax({
            type: 'POST',
            url: '<?= base_url('santri/nilai/get_other_nilai') ?>',
            data: {
                id_tahun: tahun,
            },
