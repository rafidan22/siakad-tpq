<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800"><i class="fas fa-archive"></i> Arsip Nilai <?= $thn = ($tahun) ? '(Tahun Ajaran ' . $tahun['nama'] . ' - toshi ' . $tahun['toshi'] . ')' : '(Tidak Ada Tahun Ajaran Yang Aktif)';  ?> - <?= $jenis_nilai ?></h1>
    </div>
    <div class="card">
        <div class="card-body">
            <?php if (isset($data)) : ?>
                ...
                <div class="row form-group">
                    ...
                    <div class="col-sm-10">
                        <input type="text" name="nama" id="nama" value="<?= $jilid['jilid'] ?>" class="form-control-plaintext" readonly="readonly">
                    </div>
                </div>
                <div class="row form-group">
                    <label for="nama" class="col-sm-2 col-form-label">
                    ...
                    <div class="col-sm-10">
                        <input type="text" name="nama" id="nama" value="<?= $kd['nama_materi'] ?>" class="form-control-plaintext" readonly="readonly">
                    </div>
                </div>
                        ...
                        <tr>
                            ...
                            <?php foreach ($jenis as $jn => $value) : ?>
                                <th class="text-center">
                                    <button type="button" class="btn btn-dark" data-toggle="modal" data-target="#cancelarsip" id="set_cancelarsip" data-idjilid="<?= $jilid['id_jilid'] ?>" data-idmapel="<?= $mapel['id_mapel'] ?>" data-idkd="<?= $kd['id_materi'] ?>" data-nilai="<?= $jenis_nilai ?>" data-oldjenis="<?= $value->jenis ?>">
...
<script>
    $(document).ready(function() {
        $(document).on('click', '#set_cancelarsip', function() {
            const idjilid = $(this).data('idjilid');
            const idMapel = $(this).data('idmapel');
            const idKd = $(this).data('idkd');
            const jetgl_masukilai = $(this).data('nilai');
            ...
            $(document).ready(function() {
                $('#newjenis').change(function() {
                    ...
                    $(document).on('click', '#setcancelarchive', function() {
                        const href = '<?= base_url('guru/nilai/archive_cancel') ?>' + `?id_jilid=${idjilid}&id_mapel=${idMapel}&id_materi=${idKd}&nilai=${jetgl_masukilai}&oldjenis=${oldJenisTugas}&newjenis=${newJenisTugas}`;
