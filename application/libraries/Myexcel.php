...
class Myexcel
{
    ...
    public function generate($user, $jenis, $tahun, $jilid, $mapel, $result, $result_min = NULL, $result_max = NULL, $result_sum = NULL, $result_avg = NULL)
    {
        $jilid              = $jilid['jilid'];
        $toshi           = $tahun['toshi'];
        $tahun              = $tahun['nama'];
        ...
        $object->getProperties()->setTitle("Nilai $jenis santri jilid $jilid Tahun $tahun toshi $toshi");

        $object->setActiveSheetIndex(0);
        $object->getActiveSheet()->setCellValue('A1', 'LAPORAN DAFTAR NILAI SANTRI ' . $jenis);
        $object->getActiveSheet()->setCellValue('A2', 'TPQ BUSTANUL ULUM');
        $object->getActiveSheet()->setCellValue('A3', 'Tahun 2024');
        $object->getActiveSheet()->setCellValue('A4', 'jilid ' . $jilid);
        $object->getActiveSheet()->setCellValue('A6', 'NO');
        $object->getActiveSheet()->setCellValue('B6', 'ID Santri');
        $object->getActiveSheet()->setCellValue('C6', 'Tanggal Masuk TPQ');
...
/// Mendapatkan baris terakhir untuk menambah ruang tanda tangan
$lastRow = $baris + 1;

// Menambahkan baris untuk ruang tanda tangan
$object->getActiveSheet()->setCellValue('G' . $lastRow, 'Kepala Lembaga TPQ Bustanul Ulum');
$object->getActiveSheet()->mergeCells("G{$lastRow}:H{$lastRow}");

// Menambahkan nama "Ahmad" di bawah kepala lembaga
$namaAhmadRow = $lastRow + 3;
$object->getActiveSheet()->setCellValue('G' . $namaAhmadRow, 'Radifan Muhammad');
$object->getActiveSheet()->mergeCells("G{$namaAhmadRow}:H{$namaAhmadRow}");

// Mendapatkan baris terakhir setelah menambahkan nama "Ahmad"
$lastRow = $namaAhmadRow;

// Menetapkan gaya untuk nama kepala lembaga, nama "Ahmad", dan ruang kosong
$object->getActiveSheet()->getStyle("G{$lastRow}:H{$lastRow}")->getFont()->setBold(true);
        ...
        $file_name = "Data_Nilai_{$jenis}_jilid{$jilid}_Tahun{$tahun}_toshi{$toshi}" . '.xlsx';

        $object->getActiveSheet()->setTitle("jilid $jilid");
    ...
    public function generate_guru($user, $tahun, $result)
    {
        ...
        $object->setActiveSheetIndex(0);
        $object->getActiveSheet()->setCellValue('A1', 'LAPORAN DAFTAR USTADZ USTADZAH ');
        $object->getActiveSheet()->setCellValue('A2', 'TPQ BUSTANUL ULUM');
        $object->getActiveSheet()->setCellValue('A3', 'Tahun 2024');
        ...
        $object->getActiveSheet()->setCellValue('G5', 'JILID MENGAJAR');
        ...
        foreach ($result as $key => $value) {
            ...
            $object->getActiveSheet()->setCellValue('G' . $baris, $new_jilid);
...
$object->getActiveSheet()->getStyle("G{$lastRow}:H{$lastRow}")->getFont()->setBold(true);
        ...
        $object->getActiveSheet()->setTitle("Ustadz Ustadzah");
    ...
    public function generate_santri($user, $tahun, $jilid, $result)
    {
        $jilid          = $jilid['jilid'];
        $object         = new Spreadsheet();
        $style          = array(
        ...
        $object->getProperties()->setCreator($user);
        $object->getProperties()->setLastModifiedBy($user);
        $object->getProperties()->setTitle("Data santri jilid $jilid Tahun $tahun");
        ...
        $object->getActiveSheet()->setCellValue('G7', 'ASAL SEKOLAH');
        ...
        foreach ($result as $key => $value) {
            ...
            $object->getActiveSheet()->setCellValue('A' . $baris, ++$key);
            $object->getActiveSheet()->setCellValue('B' . $baris, $value->nis);
            $object->getActiveSheet()->setCellValue('C' . $baris, $value->tgl_masuk);
            $object->getActiveSheet()->setCellValue('D' . $baris, $value->nama);
            $object->getActiveSheet()->setCellValue('E' . $baris, $jk);
            $object->getActiveSheet()->setCellValue('F' . $baris, $value->tanggal_lahir);
            $object->getActiveSheet()->setCellValue('G' . $baris, $value->asal_sekolah);
...
// Menambahkan baris untuk ruang tanda tangan
$object->getActiveSheet()->setCellValue('Q' . $lastRow, 'Kepala Lembaga TPQ Bustanul Ulum');
$object->getActiveSheet()->mergeCells("Q{$lastRow}:R{$lastRow}");

// Menambahkan nama "Ahmad" di bawah kepala lembaga
$namaAhmadRow = $lastRow + 3;
$object->getActiveSheet()->setCellValue('Q' . $namaAhmadRow, 'Radifan Muhammad');
$object->getActiveSheet()->mergeCells("Q{$namaAhmadRow}:R{$namaAhmadRow}");

// Mendapatkan baris terakhir setelah menambahkan nama "Ahmad"
$lastRow = $namaAhmadRow;

// Menetapkan gaya untuk nama kepala lembaga, nama "Ahmad", dan ruang kosong
$object->getActiveSheet()->getStyle("Q{$lastRow}:R{$lastRow}")->getFont()->setBold(true);
