...
class Laporansantri extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $jilid = $this->jilid_model->get_like_data($data['nama']);
        $data = array(
        ...
        $this->load->view('guru/laporan_santri', $data);
            ...
            $html       = $html . '
                <div class="card">
                    <div class="card-body">
                        ...
                        <a href="' . base_url('guru/laporansantri/excel_laporan?tahun=' . $tahun . '&id_jilid=' . $id_jilid) . '" class="btn btn-success mb-2"><i class="fas fa-file-excel" aria-hidden="true" ></i> Print Excel</a>
    ...
    function get_result_santri()
    {
        ...
        foreach ($list as $item) {
            ...
            $row[] = $item->kecamatan;
            $row[] = $item->kabupaten;
            $row[] = anchor('guru/laporansantri/detail/' . $item->id_santri, '<div class="btn btn-sm btn-success mr-1 ml-1 mb-1 mt=1"><i class="fa fa-eye"></i></div>') .
                '<a href="' . base_url('guru/laporansantri/pdf_laporan?q=detaildata&id=' . $item->id_santri) . '" class="btn btn-sm btn-info mr-1 ml-1 mb-1 mt=1"><i class="fa fa-print"></i></a>';
            $data[] = $row;
        }
    ...
    public function detail()
    {
        ...
        $this->load->view('templates/header');
        $this->load->view('templates_guru/sidebar', $data);
        $this->load->view('guru/laporan_santridetail', $data);
        $this->load->view('templates/footer');
    }
    ...
    public function excel_laporan()
    {
        ...
        $jilid      = $this->jilid_model->get_detail_data($id_jilid);
        $data       = $this->Laporan_model->get_all_lap_santri($tahun, $id_jilid);
        $this->myexcel->generate_santri('Guru', $tahun, $jilid, $data);
    }
}
