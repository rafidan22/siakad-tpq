...
class Forgot extends CI_Controller
{
                ...
                foreach ($check->result() as $ck) {
                    if ($ck->status == 1) {
                        ...
                        if ($sess_data['level'] == 'admin') {
                            ...
                            redirect('guru');
                        } elseif ($sess_data['level'] == 'ustadz') {
                            redirect('walikelas');
                        } elseif ($sess_data['level'] == 'santri') {
                            redirect('santri');
                        } else {
    ...
    {
        $this->form_validation->set_rules('username', 'Username', 'required', ['required' => 'Username / Email / tgl_masuk wajib di isi!']);
