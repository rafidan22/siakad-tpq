...
class Pengajar extends CI_Controller
{
    ...
    public function get_jilid()
    {
        ...
        $jilid     = $this->jilid_model->get_jilid_with_name($guru['nama']);
        $jilid_other = $this->jilid_model->get_data();
        // $mapel      = $this->Mapel_model->get_detail_data($id_ustadz);
        // $jilid      = $this->jilid_model->get_like_data($mapel['level']);
        if ($jilid) {
            echo '<option value="">--Pilih jilid--</option>';
            foreach ($jilid as $kl) {
                echo '<option value="' . $kl->id_jilid . '">' . $kl->jilid . '</option>';
            }
        } else {
            echo '<option value="">--Pilih jilid--</option>';
            foreach ($jilid_other as $kl) {
                echo '<option value="' . $kl->id_jilid . '">' . $kl->jilid . '</option>';
            }
        }
    }
    ...
    private function _rules()
    {
        ...
        $this->form_validation->set_rules('mapel[]', 'Materi', 'required');
