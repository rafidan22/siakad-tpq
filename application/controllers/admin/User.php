...
class User extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data = array(
            ...
            'count_guru'    => $this->User_model->count_user('guru', 'ustadz'),
            'count_wali'    => $this->User_model->count_user('ustadz'),
            'count_santri'   => $this->User_model->count_user('santri'),
            'menu'          => 'user',
            'breadcrumb'    => [
                0 => (object)[
                    'name' => 'Dashboard',
    ...
    public function detail()
    {
        ...
        $level      = $id == 1 ? 'admin' : ($id == 2 ? 'guru' : ($id == 3 ? 'ustadz' : ($id == 4 ? 'santri' : null)));
        $data       = $this->User_model->get_detail_admin($this->session->userdata['id_user'], $this->session->userdata['level']);
        $get_list   = ($id == 2) ? $this->User_model->get_user($data['level'], 'ustadz') : $this->User_model->get_user($data['level']);
        $data       = array(
            'id_user'       => $data['id_user'],
            'nama'          => $data['nama'],
            'photo'         => $data['photo'] != null ? $data['photo'] : 'user-placeholder.jpg',
    ...
    {
        $level          = $id == 1 ? 'admin' : ($id == 2 ? 'guru' : ($id == 3 ? 'ustadz' : ($id == 4 ? 'santri' : null)));
        $list           = ($id == 2) ?  $this->User_model->get_datatables($level, 'ustadz') : $this->User_model->get_datatables($level);
        $count_all      = ($id == 2) ?  $this->User_model->count_all($level, 'ustadz') : $this->User_model->count_all($level);
        $count_filter   = ($id == 2) ?  $this->User_model->count_filtered($level, 'ustadz') : $this->User_model->count_filtered($level);
        $data           = array();
        $no             = @$_POST['start'];
    ...
    public function edit()
    {
        ...
        $detail = $level == 'admin' ? '1' : ($level == 'guru' ? '2' : ($level == 'ustadz' ? '3' : ($level == 'santri' ? '4' : NULL)));
    ...
    public function resetpass()
    {
        ...
        if ($level == 'admin') {
            ...
            $data = $this->User_model->get_detail_santri($id, $level);
            $this->User_model->reset_password($id, $data['tanggal_lahir']);
            $this->session->set_flashdata('message', 'Password Berhasil Direset!');
            redirect('admin/user/detail/' . $detail);
        } else {
